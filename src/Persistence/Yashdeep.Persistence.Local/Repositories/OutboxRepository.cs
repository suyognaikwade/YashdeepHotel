...
public class OutboxRepository : IOutboxRepository
{
    ...
    public async Task AddAsync(OutboxMessage message, CancellationToken cancellationToken = default)
    {
        ...
        await _dbContext.SyncOutboxMessages.AddAsync(message, cancellationToken);
    }

    public async Task<OutboxMessage?> GetByEventIdAsync(Guid eventId, CancellationToken cancellationToken = default)
    {
        return await _dbContext.SyncOutboxMessages.FirstOrDefaultAsync(m => m.EventId == eventId, cancellationToken);
    }

    public async Task<long> GetNextSequenceNumberAsync(Guid deviceId, CancellationToken cancellationToken = default)
    {
        long maxSeq = await _dbContext.SyncOutboxMessages
    ...
    {
        return await _dbContext.SyncOutboxMessages
